...
class nn():
    ...
    def backprop(self, X, Y):
        ...
        for j in range(self.hidden_nodes_num):
            ...
            # o gradients
            do_td_w_o_1 = np.array([np.matmul(self.H_t[i], self.W_o_2[j]) * self.o_t[i][j] * (1 - self.o_t[i][j]) for i in range(L)])
            do_td_w_o_2 = np.array([np.matmul(self.H_t[i].T, self.W_C_1[j]) * self.o_t[i][j] * (1 - self.o_t[i][j]) for i in range(L)])
        ...
        for t in range(L):
            ...
            for j in range(self.input_nodes_num):
                ...
                dg_tdw_j = np.array([2 * pad_cos_a * np.pad(X[t], (i, M - 1 - i), 'constant', constant_values=0) * conv_cos_out[i] \
                                     + 2 * pad_sin_a * np.pad(X[t], (i, M - 1 - i), 'constant', constant_values=0) * conv_sin_out[i] for i in range(M)]).T
        ...
        weight_updates = np.array([np.sum(np.array([L_array[i, j] for i in range(L)]), axis=0) for j in range(self.input_nodes_num)])
